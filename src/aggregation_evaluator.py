...
class AggregationEvaluator:
    ...
    def evaluate_aggregated_response(self, 
    ...
    def evaluate_aggregated_report(self, aggregated_result: Dict[str, Any]) -> Dict[str, Any]:
        """评估聚合报告的结果"""
        print(f"📊 评估聚合报告 {aggregated_result['report_id']}")
        
        report_evaluation = {
            'report_id': aggregated_result['report_id'],
            'total_symptoms': aggregated_result['total_symptoms'],
            'valid_symptoms': aggregated_result['valid_symptoms'],
        ...
        # 评估每个API的响应
        for api_name, api_response in aggregated_result['api_responses'].items():
            ...
            if api_response.get('success'):
                ...
                # 评估API响应
                evaluation = self.evaluate_aggregated_response(api_response, aggregated_result.get('expected_results', []))
                report_evaluation['api_evaluations'][api_name] = evaluation
                ...
                report_evaluation['api_evaluations'][api_name] = {
                    'overall_score': 0.0,
                    'precision': 0.0,
                    'recall': 0.0,
                    'overgeneration_penalty': 0.0,
    ...
    def save_aggregated_evaluation(self, aggregated_result: Dict[str, Any], 
                                 evaluation: Dict[str, Any], output_dir: Path) -> str:
        """保存聚合评估结果"""
        ...
        results_file = output_dir / f"report_{report_id}_aggregate_evaluation_{timestamp}.json"
        
        # 准备保存的数据
        evaluation_results = {
            'metadata': {
                'report_id': report_id,
                'file_path': aggregated_result['file_path'],
                'processing_timestamp': aggregated_result['processing_timestamp'],
                'evaluation_timestamp': evaluation['evaluation_timestamp'],
                'total_symptoms': aggregated_result['total_symptoms'],
                'valid_symptoms': aggregated_result['valid_symptoms'],
                'api_clients': list(aggregated_result['api_responses'].keys())
            },
            'aggregated_symptoms': aggregated_result['aggregated_symptoms'],
            'api_responses': {},
            'evaluation_summary': evaluation['summary']
        }
        
        # 标准化API响应
        for client_name, response in aggregated_result['api_responses'].items():
            parsed_response = {}
            ...
            evaluation_results['api_responses'][client_name] = {
        ...
        # 添加评估结果
        evaluation_results['api_evaluations'] = evaluation['api_evaluations']
        
        # 保存到文件
        try:
            with open(results_file, 'w', encoding='utf-8') as f:
                json.dump(evaluation_results, f, ensure_ascii=False, indent=2)
            print(f"✅ 聚合评估结果已保存到: {results_file}")
            return str(results_file)
        except Exception as e:
            print(f"❌ 保存聚合评估结果失败: {e}")
            raise
